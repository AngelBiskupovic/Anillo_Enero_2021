...
Port_container_inputs=`jq '.Containers[2].Configuracion.Port_needed' file_config.json`

IP_container_perturbations=`jq '.Containers[3].Configuracion.IP_adress_container' file_config.json`
Port_container_perturbations=`jq '.Containers[3].Configuracion.Port_needed' file_config.json`

IP_container_outputs=`jq '.Containers[4].Configuracion.IP_adress_container' file_config.json`
Port_container_outputs=`jq '.Containers[4].Configuracion.Port_needed' file_config.json`
...
#docker run -d --ip=$IP_container_inputs -p $Port_container_inputs:$Port_container_inputs-v $(pwd):/data --name inputs_client $image_name_inputs & sleep 3
#docker run -d --ip=$IP_container_perturbations -p $Port_container_perturbations:$Port_container_perturbations -v $(pwd):/data --name perturbations_client $image_name_perturbations & sleep 3
#docker run -d --ip=$IP_container_outputs -p $Port_container_outputs:$Port_container_outputs -v $(pwd):/data --name outputs_client $image_name_outputs & sleep 3
